...
export default {
  ...
  mounted () {
  	...
  	$.ajax({
		  url: 'http://localhost/diary-test/api/info.php?id='+this.$route.query.id,
		  type: 'get',
		  dataType: 'json',
		  success: res => {
		    this.loading = false
  ...
  methods: {
  	...
  	deleteInfo () {
  		$.ajax({
				url: 'http://localhost/diary-test/api/delete.php',
			  type: 'get',
			  data: {
  	...
  	saveChange () {
  		$.ajax({
				url: 'http://localhost/diary-test/api/update.php',
			  type: 'post',
			  data: {
			  	id: this.$route.query.id,
